...
- `-` または `+` 付きタイムスタンプ、もしくはタイムスタンプ末尾に `*` を付けた行（非公開エントリ）は、続き行も含めて投稿しない
...
非公開エントリのマーク方法は2通りで、いずれの場合も続き行ごと投稿対象から除外されます：(a) 時刻の直前に `-` または `+` を置く、(b) タイムスタンプ（タイムゾーン接尾辞があればその直後）に `*` を接着する。
...
YYYY/MM/DD HH:MM:SS* 非公開（タイムスタンプ末尾に *）
YYYY-MM-DDTHH:MM:SS+09:00* 非公開（TZ付き ISO 8601 形式の末尾に *）
...
| `YYYY/MM/DD HH:MM:SS* …` | 非公開（タイムスタンプ末尾の `*`）— 続き行ごと投稿しない |
| `YYYY-MM-DDTHH:MM:SS±HH:MM* …` | 非公開（TZ 付き ISO 8601 形式の末尾 `*`） |
